...
@FeignClient(name="DEPARTMENT-SERVICE",contextId = "departmentClient") //from the above annotation feignClient is fixed to one portNumber so if the port is down we cannot make a call so we're directly using name of the microservice so that if one port is down the other port can be utilized which is up
//by using the name attribute inside the @FeignClient(name="") it internally uses LOAD BALANCER
public interface departmentAPIClient {

    //we declare a method to which we are going to make a REST API Call {getDepartment()} to Department-service from employee-service.
